...
// 2. Default location: near bottom right (40px from right, 60px from bottom)
...
// 1. Only show overlay on leetcode.com/problems pages
if (!/^https?:\/\/(www\.)?leetcode\.com\/problems\//.test(window.location.href)) {
  // Do not inject overlay if not on a problem page
  // Instead of return, just do not execute the rest of the code
} else {
  overlay.innerHTML = `
    ...
    <div style="font-size: 12px; margin-top: 8px; text-align: left; background: rgba(255,255,255,0.08); border-radius: 6px; padding: 6px 8px; width: 100%;">
      ...
      <span style="display:block;margin:2px 0;"><kbd>Ctrl</kbd> / <kbd>⌘</kbd> + <kbd>B</kbd>: <span style="margin-left:2px;">Show/Hide</span></span>
      <span style="display:block;margin:2px 0;"><kbd>Ctrl</kbd> / <kbd>⌘</kbd> + <kbd>Shift</kbd> + <kbd>↑</kbd>/<kbd>↓</kbd>/<kbd>←</kbd>/<kbd>→</kbd>: <span style="margin-left:2px;">Move</span></span>
      <span style="display:block;margin:2px 0;"><kbd>Ctrl</kbd> / <kbd>⌘</kbd> + <kbd>Q</kbd>: <span style="margin-left:2px;">Next Hint</span></span>
      <span style="display:block;margin:2px 0;"><kbd>Ctrl</kbd> / <kbd>⌘</kbd> + <kbd>D</kbd>: <span style="margin-left:2px;">Prev Hint</span></span>
      <span style="display:block;margin:2px 0;"><kbd>Ctrl</kbd> / <kbd>⌘</kbd> + <kbd>M</kbd>: <span style="margin-left:2px;">Toggle Translucency</span></span>
    </div>
  `;
  ...
  overlay.addEventListener('mousedown', (e) => {
    ...
    isDragging = true;
    // Calculate offset for both left/right and top/bottom
    if (overlay.style.left !== 'unset') {
      offsetX = e.clientX - overlay.offsetLeft;
    } else {
      offsetX = overlay.offsetWidth - (window.innerWidth - e.clientX - parseInt(overlay.style.right, 10));
    }
    if (overlay.style.top !== 'unset') {
      offsetY = e.clientY - overlay.offsetTop;
    } else {
      offsetY = overlay.offsetHeight - (window.innerHeight - e.clientY - parseInt(overlay.style.bottom, 10));
    }
  });

  document.addEventListener('mousemove', (e) => {
    if (isDragging) {
      // Revert: Dead stop at border (no halfway off-screen)
      const minLeft = 0;
      const minTop = 0;
  ...
  function updateButtons() { prevBtn.disabled = currentIndex <= 0; nextBtn.disabled = currentIndex + 1 >= fetchedHints.length && currentStage > 5; applyDisabledStyle(prevBtn); applyDisabledStyle(nextBtn); }
  ...
  // 2. Show backend autosleep warning if hint takes >10s to load, remove if hint arrives
  let backendSleepTimeout = null;
  let backendSleepMsg = null;
  nextBtn.onclick = async () => {
    ...
    // Set backend autosleep warning timeout
    if (backendSleepTimeout) clearTimeout(backendSleepTimeout);
    backendSleepTimeout = setTimeout(() => {
      if (!backendSleepMsg) {
        backendSleepMsg = document.createElement('div');
        backendSleepMsg.style.color = '#ff4d4f';
        backendSleepMsg.style.background = 'rgba(255,0,0,0.08)';
        backendSleepMsg.style.border = '1px solid #ff4d4f';
        backendSleepMsg.style.borderRadius = '6px';
        backendSleepMsg.style.padding = '8px 12px';
        backendSleepMsg.style.marginTop = '10px';
        backendSleepMsg.style.fontSize = '14px';
        backendSleepMsg.style.textAlign = 'center';
        backendSleepMsg.textContent = 'Backend is waking up (autosleep). Please refresh and try again in 1-2 min. This will not happen again soon.';
        btnContainer.parentNode.insertBefore(backendSleepMsg, btnContainer.nextSibling);
      }
    }, 10000);
    try {
      ...
      // Remove backend sleep message if present
      if (backendSleepTimeout) clearTimeout(backendSleepTimeout);
      if (backendSleepMsg && backendSleepMsg.parentNode) { backendSleepMsg.parentNode.removeChild(backendSleepMsg); backendSleepMsg = null; }
    } catch (err) {
      ...
      // Remove backend sleep message if present (if hint loaded or error)
      if (backendSleepTimeout) clearTimeout(backendSleepTimeout);
    }
  };
  ...
  window.addEventListener('keydown', (e) => {
    ...
    if ((e.ctrlKey || e.metaKey) && e.shiftKey && !e.altKey) {
      let left = overlay.style.left !== 'unset' ? parseInt(overlay.style.left, 10) : window.innerWidth - parseInt(overlay.style.right, 10) - overlay.offsetWidth;
      let top = overlay.style.top !== 'unset' ? parseInt(overlay.style.top, 10) : window.innerHeight - parseInt(overlay.style.bottom, 10) - overlay.offsetHeight;
      const moveAmount = 40;
      const halfWidth = overlay.offsetWidth / 2;
      const halfHeight = overlay.offsetHeight / 2;
